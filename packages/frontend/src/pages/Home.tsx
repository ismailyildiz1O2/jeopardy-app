...
export default function Home() {
  ...
  const handleCreate = useCallback(async () => {
    setIsCreating(true);
    try {
      const data = await api.createGame({ title: 'Yeni Jeopardy Oyunu' });
      toast.success('Oyun oluşturuldu!');
      navigate(`/game/${data.game.id}/edit`);
    } catch (err) {
      console.error('Failed to create game:', err);
      toast.error('Oyun oluşturulamadı. Lütfen tekrar deneyin.');
    } finally {
      setIsCreating(false);
    }
  }, [navigate]);
  ...
  const handleJoin = useCallback(async () => {
    ...
    if (!code) {
      toast.error('Lütfen bir oyun kodu girin');
      return;
    }
      ...
      >
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full
            ...
            <span className="text-xs font-medium text-white/50 uppercase tracking-wider">
              Sınıflar İçin İnteraktif Bilgi Yarışması
            </span>
          </div>
          ...
          <p className="text-sm sm:text-base text-white/40 max-w-md mx-auto">
            Sorularınızı oluşturun, paylaşım koduyla öğrencilerinizi davet edin
            ve gerçek zamanlı oynayın!
          </p>
        </motion.div>
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 text-left">
                ...
                <p className="text-xs text-white/40">
                  7 kategori, 42 soru ile kendi oyununuzu hazırlayın
                </p>
              </div>
              <motion.div
                animate={{ x: [0, 5, 0] }}
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-white/20 uppercase tracking-wider">veya</span>
            <div className="flex-1 h-px bg-white/5" />
          </div>
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              <div className="text-left">
                ...
                <p className="text-xs text-white/40">
                  Paylaşım kodunu girerek mevcut bir oyuna katılın
                </p>
              </div>
            </div>
        ...
        >
          Gerçek zamanlı • İşbirlikçi • Eğlenceli
        </motion.p>
      </motion.div>
    </div>
  );
